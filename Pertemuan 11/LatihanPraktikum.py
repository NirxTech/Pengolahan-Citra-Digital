...
def latihan_1():
    ...
    print("\nMatching Results:")
    if 'SIFT' in results and len(kp1_sift) > 0 and desc1_sift is not None and desc2_sift is not None:
        matches_sift = demonstrate_feature_matching(desc1_sift, desc2_sift, kp1_sift, kp2_sift, "SIFT")
    
    if 'SURF' in results and len(kp1_surf) > 0 and desc1_surf is not None and desc2_surf is not None:
        matches_surf = demonstrate_feature_matching(desc1_surf, desc2_surf, kp1_surf, kp2_surf, "SURF")
    
    if 'ORB' in results and len(kp1_orb) > 0 and desc1_orb is not None and desc2_orb is not None:
...
def latihan_2():
    ...
    # Jika tidak ada descriptor, skip latihan 2
    if len(all_descriptors) == 0:
        print(f"Total images: {len(images)}")
        print("No features extracted from images. Skipping Bag of Visual Words.")
        return
