...
def create_synthetic_dataset(base_dir="dataset"):
    """Mengunduh objek dari internet dan membuat variasinya. Mengembalikan daftar kategori yang sukses."""
    ...
    valid_categories = [] # Menyimpan kategori yang berhasil diunduh agar program tidak crash
    
    for cat in categories:
        cat_dir = os.path.join(base_dir, cat)
        os.makedirs(cat_dir, exist_ok=True)
        ref_path = os.path.join(cat_dir, "reference.jpg")
        
        # 1. Unduh Citra Referensi
        try:
            download_image(image_urls[cat], ref_path)
            ref_img = cv2.imread(ref_path)
            
            if ref_img is None:
                raise Exception("Gambar terunduh namun corrupt/gagal dibaca OpenCV.")
        ...
        cv2.imwrite(os.path.join(cat_dir, "test_scale.jpg"), pad_img)
        
        blur_img = cv2.GaussianBlur(ref_img, (5, 5), 0)
        dark_img = cv2.convertScaleAbs(blur_img, alpha=0.5, beta=-20)
        cv2.imwrite(os.path.join(cat_dir, "test_illumination.jpg"), dark_img)
        
        occ_img = ref_img.copy()
        cv2.rectangle(occ_img, (0, 0), (180, 180), (120, 120, 120), -1) 
        cv2.imwrite(os.path.join(cat_dir, "test_occlusion.jpg"), occ_img)
        
        # Jika semua berhasil, masukkan ke daftar kategori valid
        valid_categories.append(cat)

    print(f"[INFO] Dataset ({len(valid_categories)} Kategori Valid + 4 Variasi Uji) berhasil dikonfigurasi.\n")
...
class BagOfVisualWords:
    ...
    def transform_dataset(self, image_paths):
        features = []
        for path in image_paths:
            img = cv2.imread(path)
...
def main(show_visuals=False):
    ...
    # Ambil HANYA kategori yang berhasil diunduh (Pencegahan Error Mismatch)
    categories = create_synthetic_dataset(base_data_dir)
    ...
    for m_name in methods:
        ...
        for cat in categories:
            ref_path = os.path.join(base_data_dir, cat, "reference.jpg")
            ref_img = cv2.imread(ref_path)
            
            if ref_img is None:
                continue # Perlindungan ganda jika citra referensi tetap gagal dibaca
            ...
            for test_file in os.listdir(os.path.join(base_data_dir, cat)):
                if not test_file.endswith(".jpg") or test_file == "reference.jpg": 
                    continue
                    
                test_path = os.path.join(base_data_dir, cat, test_file)
    ...
    all_sift_des = []
    for cat in categories:
        img = cv2.imread(os.path.join(base_data_dir, cat, "reference.jpg"))
        if img is not None:
